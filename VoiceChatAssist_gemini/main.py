    ...
    while True:
        ...
        try:
            ...
            response = client.models.generate_content(
                model=MODEL,
                contents=[source+"的通話音檔", myfile],
                #systemInstruction="目前正在面試 你需要協助我通過科技公司的面試 你現在要提供我講稿 能夠直接念給面試官廳的講稿 並多講一些其他相關內容",
                config=types.GenerateContentConfig(
                    #system_instruction="目前正在面試 你需要協助我通過科技公司的面試 你現在要提供我講稿 能夠直接念給面試官聽的講稿 並多講一些其他相關內容",
                    system_instruction="你幫我把將語音轉成文字",
            ...
            formatted_text = f"[{source}] :  {response.text}"
            formatted_text2 = f"gemini : \n {response2.text}"

            conversation_history.append(types.Content(role="user", parts=[types.Part(text=formatted_text)]))
            print(formatted_text)
            conversation_history.append(types.Content(role="model", parts=[types.Part(text=response2.text)]))
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("按 's' 鍵開始錄音，按 's' 鍵保存並處理音檔（僅保存有語音的片段），按 Ctrl+C 結束")
