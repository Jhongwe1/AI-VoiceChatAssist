...
class AudioRecorder:
    def __init__(self, queue, save_dir, device_id=None, source_label="default", silence_threshold=0.0005, silence_duration=0.3, min_speech_duration=0.5):
        ...
        self.max_speech_duration = 15.0  # Maximum speech duration before forcing save
        ...
        if volume > self.silence_threshold:
            ...
            if self.silence_start is None:
                ...
                if speech_duration >= self.min_speech_duration:
                    print(f"檢測到停頓 ({self.source_label})，準備保存語音片段")
                    self.save_buffer()
                else:
    ...
    def start(self):
        self.recording = True
        print(f"🎙 準備錄音 ({self.source_label})，按住 's' 鍵開始錄音")
            ...
            if keyboard.is_pressed("s"):
                ...
                try:
                    with sd.InputStream(
                        ...
                        while keyboard.is_pressed("s") and self.recording:
                            time.sleep(0.01)
